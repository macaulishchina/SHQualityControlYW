 ...
 * 创建时间： 2017/9/19
 * 版权： 江苏远大信息股份有限公司
 * 描述： com.sinoyd.frame.views
 */
public class FrmViewDragHelper {
     ...
     */
    public static FrmViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new FrmViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static FrmViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final FrmViewDragHelper helper = create(forParent, cb);
     ...
     * @param forParent Parent view to monitor
     */
    private FrmViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
    ...
    /**
     * 初始化（模拟）Down事件参数，预备processTouchEvent
     * @param startX
     * @param startY
     * @param startPointId
     */
    public void processTouchDownEvent(int startX, int startY, int startPointId){
        cancel();
        final float down_x = startX;
        final float down_y = startY;
        final int down_pointerId = startPointId;
        final View down_toCapture = findTopChildUnder((int) down_x, (int) down_y);

        saveInitialMotion(down_x, down_y, down_pointerId);

        // Since the parent is already directly processing this touch event,
        // there is no reason to delay for a slop before dragging.
        // Start immediately if possible.
        tryCaptureViewForDrag(down_toCapture, down_pointerId);

        final int down_edgesTouched = mInitialEdgesTouched[down_pointerId];
        if ((down_edgesTouched & mTrackingEdges) != 0) {
            mCallback.onEdgeTouched(down_edgesTouched & mTrackingEdges, down_pointerId);
        }
    }
